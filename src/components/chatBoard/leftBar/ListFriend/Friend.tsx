...
export function Friend({friendInfo, unReadLength}: IFriendProps, ) {
  ...
  const handleSelectUser = ()=>{
    if(participation){
      ...
      socket.emit("choose_conversation",JSON.stringify({
        user_id: user._id,
        conversation_id: _id
      }));
      ...
      <div className="w-full">
        <div className="flex justify-between gap-8 w-full items-center">
          ...
          <div>
            ...
            {
              unReadLength === 0 ? null : <div className="text-white bg-red-500 rounded-full flex w-4 h-4 mx-auto justify-center items-center" style={{
                fontSize: "10px"
              }} >{
                unReadLength
              }</div>
