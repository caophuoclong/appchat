...
export default function ShowFriends({}: Props) {
  ...
  return (
    <div className="">
        <button className="relative" onClick={()=>{dispatch(setSelectedModal('showFriends'))}}>
            <FaUserFriends size="24px"/>
            {numberOfFriendRequest !== 0 && <div className="absolute text-xs bg-red-500 text-gray-300 p-1 rounded-full w-5 h-5 top-0 right-0 translate-x-2 -translate-y-2">
                {numberOfFriendRequest}
                </div>
            }
        </button>
        { selectedModal === "showFriends" && <Modal customStyle={customStyle} heading={<div>
      {lang === "en" ? "Friends" : "Danh sách bạn bè"}
        ...
        <div  className="flex p-1 gap-4 bg-green-200 my-2 rounded-2xl">
            ...
            <button onClick={()=>setSelectView("requested")}  className={`relative cursor-pointer w-1/3 rounded-2xl transition-colors duration-500 text-center py-1 ${selectView === "requested" && "bg-glareGreen"}`}>
