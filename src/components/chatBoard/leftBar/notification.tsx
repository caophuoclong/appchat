...
function Content({lang, type ,children}: {lang: string, type: string, children: string}){
    if(type === "makeFriend")
    return(
        <>  
            <span className="font-semibold text-md">{children}</span>
            <p className="text-sm pr-6">{lang === "en" ?  " sent you friend request" :" đã gửi lời mời kết bạn cho bạn."}</p>
        </>
    )
    ...
    return(
        ...
        <span className="font-bold">{children}</span>
            <p className="text-sm pr-6">{lang === "en" ?  " have accepted your friend request" :" đã đồng ý lời mời kết bạn của bạn."}</p>
        </>
    )
    else
    return(
...
export default function Notification({}: Props) {
    ...
    React.useEffect(()=>{
        setNumberOfNonSeenNoti(notifications.filter(notification => notification.seen === false).length)
    },[notifications])
  ...
  return (
    <div className="px-4 relative">
        ...
        <div ref={modal} className="absolute top-0 -right-2 translate-x-full w-96 h-52 bg-white shadow-2xl transition-all rounded-3xl border border-gray-300 visible p-2 overflow-auto">
        {
                ...
                <div className="ml-10">
                    <Content type={notification.type} lang={lang}>{notification.user.name}</Content>
                    <p className="text-xs text-blue-500">{moment(notification.date).fromNow()}</p>
                </div>
               { !notification.seen && <div className="absolute w-5 h-5 bg-blue-500 rounded-full right-0"></div>}
            </div>
            )
        }    
        </div>
